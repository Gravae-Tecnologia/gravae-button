...
async function main() {
  ...
  try {
    ...
    console.log(
      "postbuild: config.js ausente, tentando gerar a partir de src/config.ts..."
    );
    ...
    try {
      ...
      await fs.writeFile(
        dest,
        content.replace(/export type [^;]+;\n?/g, ""),
        "utf8"
      );
      console.log(
        "postbuild: escreveu dist/config.js a partir de src/config.ts (fallback)"
      );
    } catch (err) {
